...
public class PIMsteps {
    ...
    public void user_should_see_employee_list_table_or_search_panel() {
        Assert.assertTrue(pimPage.isSearchPanelDisplayed(), "PIM page search panel is not displayed.");
    }
    ...
    public void user_enters_employee_details(String fName, String mName, String lName) {
        firstName = fName;
        middleName = mName;
        // Generate unique last name to avoid collisons
    ...
    public void user_saves_new_employee_record() {
        ...
        System.out.println("BDD Test Employee Created with ID: " + capturedEmpId);
    }

    @Then("Employee should be saved and Employee ID should be captured")
    public void employee_should_be_saved_and_captured() {
        Assert.assertNotNull(capturedEmpId, "Employee ID was not generated/captured.");
        Assert.assertFalse(capturedEmpId.isEmpty(), "Employee ID is empty.");
    }
    ...
    public void user_should_find_employee_in_results() {
        Assert.assertTrue(pimPage.getSearchResultsCount() >= 1, "Employee ID not found in PIM search.");
        Assert.assertEquals(pimPage.getFirstRowEmployeeId(), capturedEmpId, "Search result Employee ID mismatch.");
    }
    ...
    public void employee_nickname_should_be_updated(String expectedNickname) {
        ...
        Assert.assertEquals(pimPage.getNickNameValue(), expectedNickname, "Nickname was not updated successfully.");
    }
    ...
    public void user_should_not_find_employee_in_results() {
        Assert.assertEquals(pimPage.getSearchResultsCount(), 0, "Deleted employee record still exists.");
    }
